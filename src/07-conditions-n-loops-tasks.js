...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let resStr = '';
  if (isStartIncluded) resStr += '[';
  else resStr += '(';

  if (a < b) resStr += `${a}, ${b}`;
  else resStr += `${b}, ${a}`;

  if (isEndIncluded) resStr += ']';
  else resStr += ')';
...
function isCreditCardNumber(ccn) {
  ...
  let sum = 0;
  const startPoint = ccnStr.length % 2;
  if (startPoint) sum += +ccnStr[0];
  for (let i = startPoint; i < ccnStr.length; i += 1) {
    let newChar = +ccnStr[i];
    if (i % 2 === startPoint) {
      newChar *= 2;
      if (newChar > 9) newChar -= 9;
    }
    sum += newChar;
  }
  return sum % 10 === 0;
